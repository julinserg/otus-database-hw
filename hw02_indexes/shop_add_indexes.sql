...
CREATE INDEX product_producer_price_idx ON Product(ProducerID, Price);

-- индекс для полтотекстового поиска продуктов по названию
CREATE INDEX product_name_en_idx ON Product USING GIN(to_tsvector('english', Name));
CREATE INDEX product_name_ru_idx ON Product USING GIN(to_tsvector('russian', Name));

-- индекс для фильтра продуктов по категориям
-- фактически это обратный индекс к индексу первичного ключа (ProductID, CategoryID)
CREATE INDEX categorytoproduct_category_idx ON CategoryToProduct(CategoryID);
...
CREATE INDEX providertoproduct_provider_idx ON ProviderToProduct(ProviderID);

-- индекс для фильтра/сортировки заказов по дате
CREATE INDEX shoporder_orderdate_idx ON ShopOrder(OrderDate);
...
CREATE INDEX shoporder_сustomer_orderdate_idx ON ShopOrder(CustomerID, OrderDate);
